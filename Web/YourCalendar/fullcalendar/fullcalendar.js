...
(function ($, undefined) {
        ...
        if (inUsers != "") {
			for (var i = 0; i <= inUsers.length; i++) {
				if (inUsers[i] != null) {
					var user = "";
					var color = "#2EB88A";
					if ((inUsers[i].first_name == null) && (inUsers[i].last_name == null)) {
						user = "Open";	
					} else if ((inUsers[i].first_name == null) && (inUsers[i].last_name != null)) {
						user = inUsers[i].last_name;
					} else {
    ...
    function Calendar(element, options, eventSources) {
		...
		function buildRedirectionURL(day,month,year,direction) {
			var retURL = "";
			var baseURL = "/YourCalendar/";
			...
			if (view.name == "month") {
				...
				retURL = baseURL + "?month=" + month + "&year=" + year
			} else {
				var futureDayStart = "";
				if ((getParameterByName("day") == 0) || (getParameterByName("day") == undefined)) {
					futureDayStart = 1;
				} else {
					if (view.name == "basicWeek") {
						if (direction == "prev") {
							view.start.setDate(view.start.getDate() - 7);
							tmpday = view.start.getDate();
							tmpmonth = view.start.getMonth();
							tmpyear = view.start.getFullYear();
							tmpmonth = parseInt(tmpmonth) + 1;
							
							if ((tmpmonth < month) && (month != "") && (month != undefined)) {
								month--;	
							}
							
							if ((tmpyear < year) && (year != "") && (year != undefined)) {
								year--;	
							}
							futureDayStart = tmpday;
						} else {
							view.end.setDate(view.end.getDate() + 1);
							tmpday = view.end.getDate();
							tmpmonth = view.end.getMonth();
							tmpyear = view.end.getFullYear();
							tmpmonth = parseInt(tmpmonth) + 1;
				...
				retURL = baseURL + "?month=" + month + "&year=" + year + "&day=" + futureDayStart;
			}
        ...
        function prev() {
			...
			window.location = buildRedirectionURL(day,month,year,"prev");
        }
        ...
        function next() {
			...
			var month = getParameterByName("month");
            var year = getParameterByName("year");
			window.location = buildRedirectionURL(day,month,year,"next");
        }
    ...
    function DayEventRenderer() {
        ...
        function daySegHTML(segs) { // also sets seg.left and seg.outerWidth
            ...
            for (i = 0; i < segCnt; i++) {
				...
				var cursor = "";
				if (skinCss.indexOf("fffc5c") > -1) {
					cursor = "pointer";
				}
				
				if (role == "Admin") {
					html += "<div class='div" + event.scheduleId + event.id + "' onclick=\"displayPopup('" + event.scheduleId + "','" + event.id + "',this)\">"; 
				} else {
					var curDate = new Date();
					var m = curDate.getMonth() + 1;
					var d = curDate.getDate();
					
					html += "<div style=\"cursor: " + cursor + "\" onclick=\"tradeShift('" + event.scheduleId + "','" + event.id + "',this)\">";
				}
				html += "<div id=\"outerDiv_" + event.scheduleId + event.id + "\"" +
                ...
                if (!event.allDay && seg.isStart) {
					...
					"&nbsp; " + htmlEscape(formatDates(event.start, event.endreal, opt('timeFormat'))) + " - " + getTimeFromDate(event.endreal) + "</div>" +
