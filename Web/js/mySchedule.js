...
function displayPopup(scheduleId, shiftId, sessionId, grpcode, divId, role) {
	...
	source = source + '<div style="text-align: left;">';
		...
		if (shiftObj.users != null) {
			for (var i in shiftObj.users) {
				if (shiftObj.users[i].id == "NOPE") {
	...
	$.post('ws/addUserToShift', {"sessionId":sessionId,"grpcode":grpcode,"orgId":id,"org_name":username,"username":selectedUser,"scheduleId":scheduleId,"shiftId":shiftId} , function(data) {
		...
		if ((message == "success") || (message == "warning")) {
			...
			if (numLeft == 1) {
				...
				if ($('#'+id+' .userDisplay').html().toLowerCase() == "open") {
...
function displayStatsPopup(sessionId, grpcode, scheduleId, shiftId) {
	...
	displayStatsPopup2(sessionId, grpcode, scheduleId, shiftId, user);
}

//This method initialises the modal popup
function displayStatsPopup2(sessionId, grpcode, scheduleId, shiftId, user) {
	var stats;
	...
	$.post('ws/getScheduleStats', {"sessionId":sessionId,"grpcode":grpcode,"month":m,"year":y,"user":user} , function(data) {
...
function publishSchedule(sessionId,grpcode,scheduleId) {
	if (confirm("Are you sure you would like to publish this schedule?  After publishing, the schedule will be viewable by all providers.")) {
		$.post('../ws/publishSchedule', {"sessionId":sessionId,"grpcode":grpcode,"scheduleId":scheduleId} , function(data) {
