...
$scheduleId = $schedule['data']['1']['scheduleId'];
...
<script language="javascript"> 
	...
	function getUsers() {
		var userObj = "";
		$.post('ws/getUsers', {"sessionId":"<?=session_id()?>","grpcode":"<?=$_SESSION['grpcode']?>"} , function(data) {
			userObj = data.data;
		});
	...
	function processHighlightRequest() {
		var value = $('#ddlUsers :selected').val();
		...
		if (value == "") {
			...
			$('[id*=' + value + ']').css("background-color","#FFFFCC");
		}
	}
        ...
        <div id="sticky_navigation">
            <div class="demo_container" style="text-align: center;">
                <ul>
                    ...
                    <li><span style="cursor: pointer; border-bottom: 1px dotted #CCC;" onClick="regenerateSchedule('<?=session_id();?>','<?=$_SESSION['grpcode'];?>','<?=$GLOBALS['scheduleId'];?>')"><a name="regen" href="">Generate Schedule</a></span></li>
					...
					//print_r($response);
										...
										if ($shiftDays != false) {
										 ...
										 while ( $day_num <= $days_in_month ) { 
											...
											foreach ($shiftDays[$day_num] as $shift) {
												...
												// If there are users within the shift, display the shift once for every user, along with the username
												if ($GLOBALS['openOnly'] != "1") {
								...
								function displayOpenShift($shift,$start,$end,$day_num) {
									if ($shift['users'] == "") {
										...
										echo '<div onclick="displayPopup(\'' . $GLOBALS['scheduleId'] . '\', \'' . $shift['shiftNum'] . '\', \'' . session_id() . '\', \'' . $_SESSION['grpcode'] . '\', \'' . $day_num . "_" . $shift['id'] . "_Open" . '\', \'' . $GLOBALS['role'] . '\')" removeId="' . $day_num . "_" . $shift['id'] . '" class="calShift" style="background-color: #CC0000; color: #FFF;" id="' . $day_num . "_" . $shift['id'] . "_Open" . '">';
											echo '<div class="userDisplay Open">';
												echo "OPEN";
											echo '</div>';
											echo $shift['name'] . $timeString;
