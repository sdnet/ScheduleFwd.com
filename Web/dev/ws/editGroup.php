...
if(VerifySession($sessionId,$groupcode,false,false) == true){
	...
	if(($groupId == "" || $groupname == "" || $groupcode == "" || !isset($max) || !isset($min))) {
			$message =  "emptyFields";
	} else {
		$hourCheck = hourCheck($min,$max);	
		if($hourCheck){
			$where = array('name' => "$groupname", '_id' => array('$ne' => new MongoId($groupId)));
			...
			if($results == null){
				...
				$data = $db->upsert(array('id' => $groupId, 'col' => $groupcode, 'type' => "group", 'obj' => $obj ));
				$message = "success";
			}else{
				$message =  "groupExists";	
			}
