...
class Staff {
    ...
    private function getFirstAvailableShift($shiftId, $offset = 0, $weekend = false) {
        return getFirstAvailableShift($this->schedule, $shiftId, $offset, $weekend);
    }
        ...
        foreach ($this->users as $user) {
            ...
            while ($canUserTakeShift == false) {
                ...
                while ($goodshift == false) {
                    ...
                    $nextShift = $this->getFirstAvailableShift($shiftId, $shiftpass);
        ...
        foreach ($this->users as $user) {
            ...
            // user preference and the user can work it 
                ...
                while ($canUserTakeShift == false) {
                    ...
                    while ($goodshift == false) {
                        // Gets the next instance of the user-preferred shift
                        $nextShift = $this->getFirstAvailableShift($shiftId, $shiftpass,'8');
                        if ($this->isUserOverMax($user,$nextShift)) {
                            break 2;
